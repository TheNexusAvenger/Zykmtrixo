...
namespace Zykmtrixo.Diagnostic;
...
public class Logger
{
    ...
    {
        IncludeDate = true,
        NamespaceWhitelist = new List<string>() { "Zykmtrixo" },
    ...
    {
        if (logLevel == ConsoleOutput.MinimumLevel) return;
        ConsoleOutput.MinimumLevel = logLevel;
        Logger.Debug($"Set minimum log level to {logLevel}.");
    }
